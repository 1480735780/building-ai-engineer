...
```Plain Text
┌──────────────────────────────────────────────┐
               │         完整原始文档 (Full Document)          │
               └──────────────────────┬───────────────────────┘
                                      │
              ┌───────────────────────┴───────────────────────┐
              ▼                                               ▼
     [ 传统 Chunk 1 ]                                [ 传统 Chunk 2 ]
              │                                               │
              ├───────────────────────┐                       │
              ▼                       ▼                       │
    ┌──────────────────┐    ┌──────────────────┐              │
    │  轻量 LLM Prompt  │    │  轻量 LLM Prompt  │             │
    │ (带 Prompt Cache)│    │ (带 Prompt Cache)│              │
    └─────────┬────────┘    └─────────┬────────┘              │
              ▼                       ▼                       │
   生成 50-100 字 Context    生成 50-100 字 Context             │
              │                       │                       │
              ▼                       ▼                       ▼
   ┌─────────────────────────────────────┐         ┌────────────────────┐
   │ Contextualized Chunk 1              │         │ Contextualized...  │
   │ [全局背景前缀] + [原始 Chunk 1 内容]   │         └────────────────────┘
   └──────────────────┬──────────────────┘
                      │
            ┌─────────┴─────────┐
            ▼                   ▼
    [ 向量数据库 Vector DB ]   [ BM25 倒排索引库 ]  <-- 双轨同步增强！
```
...
针对第一点，其实要是chunk数量达到百万级别，那么需要LLM对每个Chunk进行总结概述一个prompt的化，所消耗的Token将不可估计。不仅成本太高，而且太慢了！
...
在理想的实验环境里，我们假设用户会像写 Prompt 一样，输入结构完整、逻辑严密、专有名词对齐的查询语句。但在真实的真实业务（如 WMS 仓储管理系统、企业内部 KB）中，**真实用户的 Query 往往是一团糟的。**
...
- **原理**：利用**正则表达式**、**关键词词典匹配**强特征（如设备 ID、订单号、错误代码）。【Sparse Retrieval】
...
```Python
【Rewrite】    【2. Multi-Query】 【3. Decomp】   【4. HyDE】   【5. Step-Back】  
...
2. **Fan\-out Async Engine**：利用 `asyncio.gather` 并发向向量库/搜索引擎发起 $k$ 路独立检索。
...
2. 结果融合：使用倒数排名融合（RRF） 等算法合并多路检索结果，避免高质量文档被淹没
...
### 第⑤章 Advanced Retriever Architecture

#### 5\.1 SelfQueryRetriever\(自然语言转结构化元数据过滤 \(AST Filter\)

#### 5\.2 MultiVectorRetriever\(摘要、假设问题与多模态表征映射\)
